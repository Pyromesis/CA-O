# CA-O 2.1.19 — Plataforma nativa de rendimiento, diagnóstico y optimización para Windows 11
...
**Descargas v2.1.19:** [CA-O-Instalador.exe (135 MB, un solo exe con ventana: descarga y abre el instalador)](https://github.com/Pyromesis/CA-O/releases/download/v2.1.19/CA-O-Instalador.exe) | [CA-O-Setup-GUI-x64.zip (92 MB, instalador GUI)](https://github.com/Pyromesis/CA-O/releases/download/v2.1.19/CA-O-Setup-GUI-x64.zip) | [CA-O-2.1.19-win-x64.zip (394 MB, paquete completo offline)](https://github.com/Pyromesis/CA-O/releases/download/v2.1.19/CA-O-2.1.19-win-x64.zip) | [Notas de la versión](https://github.com/Pyromesis/CA-O/releases/tag/v2.1.19) | [Documentación](docs/ARCHITECTURE.md)
...
|---|---|---|
| `CA-O.Shared` | DTOs, contratos IPC v2 (`IpcProtocol` v2, `Ping`, `GetServiceStatus`), `CaOPaths`, `ErrorCodes CAO-XXX-nnn`, `AppVersion 2.1.19`, `BuildConstants`, `Constants/IpcConstants` | — |
...
│   │   ├── Constants/               # AppVersion (2.1.19), BuildConstants, CaOPaths, IpcConstants
...
├── Version.props                    # Single source 2.1.19
...
1. Descarga [CA-O-Instalador.exe (135 MB)](https://github.com/Pyromesis/CA-O/releases/download/v2.1.19/CA-O-Instalador.exe) — es el único archivo que necesitas
2. Ejecútalo (pide UAC) → descarga el paquete (~450 MB con progreso) y abre el **instalador gráfico**, que instala app + servicio + atajos y abre CA-O
3. Necesita internet la primera vez (luego el ZIP queda en `%TEMP%\CA-O-payload.zip`)
...
3. Si no hay payload local (`ui/`+`service/` adyacentes), descarga `CA-O-2.1.19-win-x64.zip` desde GitHub Releases (requiere internet) y continúa
...
# → artifacts/CA-O-2.1.19-win-x64.zip + .sha256
...
- **Versión actual:** **2.1.19** — [Releases](https://github.com/Pyromesis/CA-O/releases/tag/v2.1.19)
- **Artefactos:**
  - `CA-O-Setup-GUI-x64.zip` (92 MB, GUI online)
  - `CA-O-2.1.19-win-x64.zip` (343 MB, paquete completo offline con `ui/`, `service/`, `gui-installer/`, `setup/`, `uninstall/`)
...
```powershell
Get-FileHash artifacts/CA-O-2.1.19-win-x64.zip -Algorithm SHA256
...
| **Error 404 al instalar con GUI** | URL obsoleta (v2.0.4 y anteriores `v2.0.1/CA-O-2.0.0-…`) | Corregido v2.1.6 con fallback a `latest`. Usa `CA-O-2.1.19-win-x64.zip` offline o `CA-O-Setup-GUI-x64.zip` con internet |
...
<p align="center"><i>CA-O 2.1.19 — Diagnóstico primero, evidencia después, transacción siempre. Sin promesas, solo hechos medibles.</i></p>
