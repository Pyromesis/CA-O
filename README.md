# CA-O 2.1.12 — Plataforma nativa de rendimiento, diagnóstico y optimización para Windows 11
...
[![Tests](https://img.shields.io/badge/tests-604%20passed-brightgreen?style=flat-square)](#-pruebas)
[![Release](https://img.shields.io/badge/release-v2.1.12-blue?style=flat-square)](https://github.com/Pyromesis/CA-O/releases/tag/v2.1.12)
[![License](https://img.shields.io/badge/license-privado-lightgrey?style=flat-square)](#licencia)

**Descargas v2.1.12:** [CA-O-Setup-GUI-x64.zip (92 MB, instalador GUI)](https://github.com/Pyromesis/CA-O/releases/download/v2.1.12/CA-O-Setup-GUI-x64.zip) | [CA-O-2.1.12-win-x64.zip (394 MB, paquete completo offline)](https://github.com/Pyromesis/CA-O/releases/download/v2.1.12/CA-O-2.1.12-win-x64.zip) | [Notas de la versión](https://github.com/Pyromesis/CA-O/releases/tag/v2.1.12) | [Documentación](docs/ARCHITECTURE.md)
...
- **604 tests** en 6 suites (Core 438 · Integration 48 · Security 73 · Infra 17 · Benchmark 7 · UI 21)
...
| **Build** | `Directory.Packages.props` + `Directory.Build.props` + `Version.props` (single source `2.1.12`) | Centralizado | Un lugar para bump de versión/paquetes |
| **Tests** | xUnit 2.9.2 + Microsoft.NET.Test.Sdk 17.11.1 | — | 604 tests en Release |
...
|---|---|---|
| `CA-O.Shared` | DTOs, contratos IPC v2 (`IpcProtocol` v2, `Ping`, `GetServiceStatus`), `CaOPaths`, `ErrorCodes CAO-XXX-nnn`, `AppVersion 2.1.12`, `BuildConstants`, `Constants/IpcConstants` | — |
...
│   │   ├── Constants/               # AppVersion (2.1.12), BuildConstants, CaOPaths, IpcConstants
...
├── Version.props                    # Single source 2.1.12
...
1. Descarga [CA-O-2.1.12-win-x64.zip (394 MB)](https://github.com/Pyromesis/CA-O/releases/download/v2.1.12/CA-O-2.1.12-win-x64.zip)
...
1. Descarga [CA-O-Setup-GUI-x64.zip (92 MB)](https://github.com/Pyromesis/CA-O/releases/download/v2.1.12/CA-O-Setup-GUI-x64.zip)
2. Descomprime → `CA-O.InstallerGui.exe` como admin
3. Si no hay payload local (`ui/`+`service/` adyacentes), descarga `CA-O-2.1.12-win-x64.zip` desde GitHub Releases (requiere internet) y continúa
...
# → artifacts/CA-O-2.1.12-win-x64.zip + .sha256
...
**604 pruebas en 6 suites, todas en Release:**

| Suite | Cubre | Cant. |
|---|---|---|
| `CA-O.Core.Tests` | Contratos catálogo (75), `AnalysisStateStore` (save/load/corrupt/schema), `GameCompatibility` (VBS bloqueado), `OptimizationTransaction` (snapshot/apply/verify/rollback), `RecommendationEngine`, `KnownIssueMatcher`, `HealthEngine`, 75 definiciones `IOptimization` | **438** |
...
| `CA-O.UI.Tests` | `ViewModelTests` (Analyze/Dashboard con `SystemAnalysisService` + `CorrelationId`), `LocalizerTests` | **21** |
| **Total** | **Gates 1-5 `verify.ps1` + `build-release` con `gui-installer`** | **604** |
...
- **Versión actual:** **2.1.12** — [Releases](https://github.com/Pyromesis/CA-O/releases/tag/v2.1.12)
- **Artefactos:**
  - `CA-O-Setup-GUI-x64.zip` (92 MB, GUI online)
  - `CA-O-2.1.12-win-x64.zip` (343 MB, paquete completo offline con `ui/`, `service/`, `gui-installer/`, `setup/`, `uninstall/`)
...
```powershell
Get-FileHash artifacts/CA-O-2.1.12-win-x64.zip -Algorithm SHA256
...
| **Error 404 al instalar con GUI** | URL obsoleta (v2.0.4 y anteriores `v2.0.1/CA-O-2.0.0-…`) | Corregido v2.1.6 con fallback a `latest`. Usa `CA-O-2.1.12-win-x64.zip` offline o `CA-O-Setup-GUI-x64.zip` con internet |
...
<p align="center"><i>CA-O 2.1.12 — Diagnóstico primero, evidencia después, transacción siempre. Sin promesas, solo hechos medibles.</i></p>
