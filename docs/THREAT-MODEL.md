...
| **Downgrade / Protocol confusion** | Version | `ProtocolVersion` 2 exacto, `AppVersion.Semantic 2.1.35` en manifest, UI y Service rechazan mismatch `CAO-IPC-001`; `ApplicationVersion`/`SchemaVersion`/`CatalogVersion` futuros a extender. | `IpcRequestValidator` + `CAO.Integration.Tests/DocumentationCodeConsistencyTests` |
