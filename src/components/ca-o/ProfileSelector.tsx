...
// Predefined profiles now live in src/lib/profiles.ts so both client and
// server code share one source of truth.
import { predefinedProfiles } from '@/lib/profiles';
import { apiFetch } from '@/lib/session-client';
export { predefinedProfiles };
  ...
  const handleConfirmApply = async (createBackup: boolean) => {
    ...
    try {
      ...
      if (createBackup && total > 0) {
        ...
        try {
          await apiFetch('/api/troubleshoot/execute', {
            method: 'POST',
