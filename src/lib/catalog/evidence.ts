...
const groupDefaults: Record<string, Omit<EvidenceRecord, 'adverseEffects'>> = {
  ...
  diagnostics: { expectedImpact: 'diagnostic-only', confidence: 'high', sources: ['microsoft'], rationaleEs: 'Interruptor de diagnóstico: no es una optimización de rendimiento.', rationaleEn: 'Diagnostic switch: not a performance optimization.' },
  ...
  maintenance: { expectedImpact: 'none', confidence: 'high', sources: ['microsoft'], rationaleEs: 'Mantenimiento o gestión de funciones; sin promesa de FPS.', rationaleEn: 'Maintenance or feature management; no FPS promise.' },
  repair: { expectedImpact: 'diagnostic-only', confidence: 'high', sources: ['microsoft'], rationaleEs: 'Acción de reparación, no de rendimiento.', rationaleEn: 'Repair action, not a performance tweak.' },
  ...
  experimental: { expectedImpact: 'diagnostic-only', confidence: 'low', sources: ['heuristic'], rationaleEs: 'Sin evidencia moderna en Windows 11; puede no aportar beneficio real o ser contraproducente según el equipo.', rationaleEn: 'No modern Windows 11 evidence; may provide no real benefit or be counterproductive depending on the machine.' },
};
...
function buildEvidenceRecord(id: string): EvidenceRecord {
  ...
  const base = groupDefaults[entry?.group ?? 'maintenance'] ?? groupDefaults.maintenance;
  return {
