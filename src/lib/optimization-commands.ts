...
const additionalRealCommands: typeof baseRealCommands = {
  ...
  'disable-keyboard-filter': { commands: [{ description: 'Disabling keyboard filter service', script: "if (Get-Service -Name WbioSrvc -ErrorAction SilentlyContinue) { Set-Service -Name WbioSrvc -StartupType Manual -ErrorAction Stop }" }], rebootRequired: false },
  ...
  'enable-core-parking': { commands: [{ description: 'Disabling processor core parking', script: 'powercfg /setacvalueindex SCHEME_CURRENT SUB_PROCESSOR CPMINCORES 100; powercfg /setactive SCHEME_CURRENT' }], rebootRequired: false },
...
export const nonExecutableReasonById: Record<string, { es: string; en: string }> = {
  ...
  'disable-keyboard-filter': { es: 'No existe un filtro de teclado global seguro para desactivar; tocar servicios biométricos no optimiza el teclado.', en: 'There is no safe global keyboard filter to disable; changing biometric services would not optimize the keyboard.' },
};
...
export const verificationCommands = {
  ...
  'disable-keyboard-filter': "if (Get-Service WbioSrvc -ErrorAction SilentlyContinue) { if ((Get-Service WbioSrvc).StartType -eq 'Disabled') { throw 'Unexpected keyboard dependency disabled' } }; Write-Output 'Keyboard filter policy checked'",
  ...
  'enable-core-parking': "if ((powercfg /query SCHEME_CURRENT SUB_PROCESSOR CPMINCORES | Out-String) -notmatch '0x00000064') { throw 'Core parking was not disabled' }",
...
export const revertVerificationCommands: Record<string, string> = {
  ...
  'enable-core-parking': "if ((powercfg /query SCHEME_CURRENT SUB_PROCESSOR CPMINCORES | Out-String) -match '0x00000064') { throw 'Core parking remains disabled' }",
...
export const revertCommands = {
  ...
  'disable-keyboard-filter': { commands: [{ description: 'Restoring keyboard filter service policy', script: "if (Get-Service -Name WbioSrvc -ErrorAction SilentlyContinue) { Set-Service -Name WbioSrvc -StartupType Manual -ErrorAction Stop }" }], rebootRequired: false },
  ...
  'enable-core-parking': { commands: [{ description: 'Restoring processor core parking', script: 'powercfg /setacvalueindex SCHEME_CURRENT SUB_PROCESSOR CPMINCORES 10; powercfg /setactive SCHEME_CURRENT' }], rebootRequired: false },
